...
bool FSpatialGDKEditorToolbarModule::StartSpatialDeploymentIsVisible() const
{
	...
	{
		return GetDefault<UGeneralProjectSettings>()->UsesSpatialNetworking() && !LocalDeploymentManager->IsLocalDeploymentRunning();
	}
	else
	{
		return GetDefault<UGeneralProjectSettings>()->UsesSpatialNetworking();
...
{
	return !LocalDeploymentManager->IsDeploymentStarting() && GetDefault<UGeneralProjectSettings>()->UsesSpatialNetworking() && !CookMapProcess.IsValid();
}
