...
void USpatialReceiver::OnComponentUpdate(const Worker_ComponentUpdateOp& Op)
{
	...
	case SpatialConstants::DEBUG_METRICS_COMPONENT_ID:
	case SpatialConstants::ALWAYS_RELEVANT_COMPONENT_ID:
		UE_LOG(LogSpatialReceiver, VeryVerbose, TEXT("Entity: %d Component: %d - Skipping because this is hand-written Spatial component"), Op.entity_id, Op.update.component_id);
		return;
	case SpatialConstants::GSM_SHUTDOWN_COMPONENT_ID:
#if WITH_EDITOR
